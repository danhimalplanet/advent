...
while i < 10000000:
    ...
    if tuple(b) in record:
        first = record[tuple(b)]['cycle']
        cycles = cycles + 1
        record[tuple(b)]['sightings'] = record[tuple(b)]['sightings'] + 1
        record[tuple(b)]['cycle'] = cycles
        difference = record[tuple(b)]['cycle'] - first
        #print(difference)
        #print(record[tuple(b)]['cycle'] - sum)
        #print("we found the reallocated banks in the dict already")
        break
    else:
        cycles = cycles + 1
        #print("added to the record", b)
        #record[tuple(b)] = { 'banks': b, 'cycle': cycles }
        record[tuple(b)] = { 'sightings': 1,'cycle': cycles }
...
#print(record)
print("redistributions:", cycles)
print("cycles:", difference)
