input_ = """pphsv ojtou brvhsj cer ntfhlra udeh ccgtyzc zoyzmh jum lugbnk
...
def is_valid_phrase(line):
    ...
    assert words
    for word in words:
        print(f"words: {words}, word: {word}, count: {words.count(word)}")
        if words.count(word) > 1:
            return False
    return True

def is_valid_phrase2(line):
    words = line.split(" ")
    ...
    for word in words_sorted:
        print(f"words: {words}, word: {word}, count: {words_sorted.count(word)}")
        if words_sorted.count(word) > 1:
            return False
    return True

def p1():
    assert is_valid_phrase("aa bb cc dd ee")
    assert not is_valid_phrase("aa bb cc dd aa")
    assert is_valid_phrase("aa bb cc dd aaa")
    ...
    print(f"I: VALID COUNT: {valid_count}")

def p2():
    assert is_valid_phrase2("a ab abc abd abf abj")
    assert not is_valid_phrase2("abcde xyz ecdab")
    assert is_valid_phrase2("iiii oiii ooii oooi oooo")
    assert not is_valid_phrase2("oiii ioii iioi iiio")

    lines = input_.split("\n")
    valid_count = 0
    for line in lines:
        if is_valid_phrase2(line):
            valid_count += 1

    print(f"II: VALID COUNT: {valid_count}")
\
p2()