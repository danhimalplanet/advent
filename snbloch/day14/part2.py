...
binary_hashes = []
for i in hashes:
    hash = bin(int(i, 16))[2:].zfill(binary_hash_length)
    binary_hashes.append(hash)

grid = empty([num_rows, binary_hash_length], dtype=int)

row_counter = 0
for i in binary_hashes:
    column_counter = 0
    while column_counter < len(str(i)):
        grid[row_counter][column_counter] = str(i)[column_counter]
        column_counter += 1
    row_counter += 1

labels, numLabels = ndimage.label(grid)
